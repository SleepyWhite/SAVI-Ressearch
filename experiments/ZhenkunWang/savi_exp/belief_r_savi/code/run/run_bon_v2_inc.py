...
[PREREG_bon32.md §1] This file is a mirror copy of run_bon_v2.py, used for the incremental
L3 sampling-pool expansion N=16→32 (16 new chains at seed=1). The only whitelisted changes:
(a) default output directory changed to outputs/bon_v2_inc/; (b) output filenames carry a
seed suffix (..._N16_seed1.jsonl); (c) this comment block. Generation parameters, prompt
construction, extraction logic, and teacher-forcing scoring are line-identical to the
original file, asserted by the diff whitelist in scripts/selfcheck_bon32_mirror.py
(including a counterexample test).

Relation to v1 (`run_generative.py`)
- prompt reuses v1's `build_prompt` byte-for-byte (imported directly, not retyped),
  guaranteeing comparability with the T=0 greedy batch
- v1 is a single do_sample=False pass; this script is an N-chain do_sample=True pool
- this script only does "sample + score + write to disk", computing no metrics; N=16/32,
  vote tie rules, and the BoN scope are all computed in `summarize_bon_v2.py` —
  changing the scope requires no GPU rerun

Scope
- sampling: temperature/top_k/top_p all explicitly specified; the model's
  generation_config defaults are not consumed (Qwen3's defaults are T=0.6/top_k=20/
  top_p=0.8 — without explicit overrides it would not be pure T=1 sampling)
- N=16 = nested subsampling of this file's first 16 chains, not independent resampling;
  the budget curve is more stable with nesting
- chain scoring: teacher-forcing recomputation, summing only over positions "in the
  generated segment and up to (and including) the first EOS"
  (generate's output_scores is not used — that would store n_steps × B × vocab logits,
  tens of GB at 1024 steps). Both sum and per-token mean scopes are given and stored on
  disk; which to use is left to analysis
- the scoring forward pass uses left padding, so position_ids are passed explicitly;
  `--selfcheck` verifies single-chain scoring matches in-batch padded scoring
...
def scenario_subset(df, n_scenarios):
    """Sample by scenario (PLAN §8: the statistical unit is the scenario; twins enter/leave together).

    Stratification key = intent(strong/weak) × BU/BM, taken from ponens rows; equidistant
    sampling within each stratum, no RNG — same parameters always give the same subset.
    Returned rows include both modus twins.
    """
...
def score_chains(model, ids, attn, gen_start, batch, vocab_chunk=256):
    """Per-token log-likelihood (generated segment only). Returns (B, L_gen) float32."""
    outs = []
...
def main():
    ...
    if args.selfcheck:
        # What is tested is the property BoN actually depends on: scoring noise must be
        ...
        #  bf16 GEMM accumulates in a different order at different batch sizes, so a
        #  ~380-token chain inherently differs by ~0.4 nat, while the between-chain gap is ~19 nat.)
        import numpy as np
